...
module.exports = (function() {
  var ConnectorManager = function(sequelize, config) {
    ...
    this.onConnectHooks = ["SET time_zone = '+0:00'"].concat(config.onConnectHooks || []);
  ...
  var connect = function(done, config) {
    ...
    var connect = Promise.promisify(connection.connect.bind(connection));
    var seq = config.onConnectHooks.map(function(hook) {
      return Promise.promisify(connection.query.bind(connection, hook));
    });

    seq.unshift(connect);

    seq.reduce(function(aggregate, factory) {
      return aggregate == null ? factory() : aggregate.then(factory);
    }, null).then(function() {
